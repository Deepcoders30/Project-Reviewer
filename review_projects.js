...
const auditProject = require("./lighthouse_audit");
...
if (
  ...
  !inputJSONData.candidate_id ||
  !Array.isArray(inputJSONData.projects)
) {
...
const reviewProject = async () => {
  ...
  for (const project of projects) {
        ...
        (async () => {
          ...
          if (response.status() === 200) {
            ...
            if (/404|not found|page not found/i.test(visibleText)) {
              is404 = true;
              throw new Error(
                "404 Not Found: The provided URL does not exist."
              );
            }
          }
          ...
          if (
            contentFound.includes("/_next/") &&
            contentFound.includes("self.__next_f.push")
          ) {
            tag = "Next JS";
          } else if (
          ...
          for (const element of elements) {
            const tagNamePresent = await element.evaluate((el) =>
          ...
          screenshotPathLocation = path.join(
            screenshotsDir,
            `${candidate_id}_${project.name}.png`
          );
          await page.screenshot({
            path: screenshotPathLocation,
            fullPage: true,
          });
        ...
        new Promise((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error(
                  `Project ${project.name} took more than 10 seconds for a review.`
                )
              ),
            10000
          )
    ...
    fs.appendFileSync(
      "timing_logs.txt",
      `-> Project "${project.name}" took ${duration}s to review.\n\n`
    ); // Logging duration
  }
